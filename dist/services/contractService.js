...
<head>
<meta charset="UTF-8" />
<title>Договор аренды № ${esc(contractNumber)} — ${esc(config_1.config.serviceName)}</title>
<style>
...
<body>
  ...
  <p>2.1. Срок аренды: с «${esc((0, dateFormat_1.formatDate)(booking.date_from))}» по «${esc((0, dateFormat_1.formatDate)(booking.date_to))}» (${esc(days)} дн.).</p>
  ...
  <p>3.3. Сервис «${esc(config_1.config.serviceName)}» предоставил исключительно информационную площадку для контакта Сторон, не является стороной настоящего Договора и освобожден от любых претензий и ответственности.</p>
  ...
  <p class="note">Документ автоматически сформирован сервисом ${esc(config_1.config.serviceDomain)} на основании данных бронирования № ${esc(contractNumber)}.
  ${esc(config_1.config.serviceName)} — информационный посредник (ст. 1253.1 ГК РФ), не является стороной сделки и не проверяет заполненные вручную поля.
